...
{
	const char* answer;			//返却する変数 for文を通過したとき何らかのポインタが代入される
	int cnt = 0;				//発見できなかったときナル文字を与えるために使います
	int len = strlen(s2);		//s2の文字数を出し strncmp関数で使います
	...
	for(int i=0; *s1; i++) {
		...
		if(*s1 == *s2) {
			...
			//訂正部分 cstringの関数を使い同じ文字列だと確認
			//できたとき
			if(strncmp(s1,s2,len) == 0) {

				answer = s1;		//返却する変数に、ポインタを代入します
									//s2と同じ文字列を発見したときなので イニシャルの K です
				//発見したのならば走査は必要ありません
				break;
			}
		}
		//文字列を走査して、文字s2の先頭文字を探します
		s1++;
	}

	//文字を発見できなかったときはナル文字を与えます
	if(cnt == 0) {

		//if文で分岐させないと、せっかく発見しても無駄になります
		answer = s1;
	}
	//いずれにしてもs1のポインタを返却します
	return answer;
}
