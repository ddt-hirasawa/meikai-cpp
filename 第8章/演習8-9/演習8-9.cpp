...
char* my_strcpy(char*,const char*);			// 関数の宣言 文字列をコピー
char* my_strncpy(char*,const char*,int n);	// 関数の宣言 長さを指定して文字列をコピー
...
char* my_strcpy(char* s1,const char* s2)
{
	...
	for(int i=0;*s2;i++){
		// s1 に s2 の文字を代入し s1 と s2 のポインタ をインクリメント
		//これで先頭から末尾まで漏れなくコピーできる
		*s1++ = *s2++;
	}
	// コピーした文字列s1 の最後尾に ナル文字を代入
