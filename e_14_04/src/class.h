...
class Matrix {
	...
	double** ptr;		//2次元配列の先頭要素へのポインタ 配列の1つの要素の、中にさらに配列を作るのでポインタのポインタ
...
public:
	...
	Matrix(int x, int y) :
		...
		ptr = new double*[_height+1];					//構築されているオブジェクト代入される ポインタが指す部分から行分
														//ポインタを作成 0 を指すとエラーが出るので要素数を1つ増やす

		*ptr = new double[(_height+1) * (_width + 1)];//構築されているオブジェクト代入される そのポインタが指す部分から
